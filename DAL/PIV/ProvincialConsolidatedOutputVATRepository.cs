...
{
    public class ProvincialConsolidatedOutputVATRepository
    {
        private readonly string _connectionString =
            ConfigurationManager.ConnectionStrings["HQOracle"].ConnectionString;

        public List<ProvincialConsolidatedOutputVATModel> GetProvincialConsolidatedOutputVAT(
            DateTime fromDate,
            DateTime toDate,
            string compId)
        {
            var result = new List<ProvincialConsolidatedOutputVATModel>();
            ...
            using (OracleCommand cmd = new OracleCommand(sql, conn))
            {
                ...
                try
                {
                    ...
                    using (OracleDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            var item = new ProvincialConsolidatedOutputVATModel
                            {
                ...
                catch (Exception ex)
                {
                    ...
                    throw new Exception("Error fetching Provincial Consolidated Output VAT report: " + ex.Message, ex);
                }
            }
