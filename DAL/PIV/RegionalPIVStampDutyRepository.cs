...
{
    public class RegionalPIVStampDutyRepository
    {
        private readonly string _connectionString =
            ConfigurationManager.ConnectionStrings["HQOracle"].ConnectionString;

        public List<RegionalPIVStampDutyModel> GetRegionalPIVStampDutyReport(
            string compID, DateTime fromDate, DateTime toDate)
        {
            var result = new List<RegionalPIVStampDutyModel>();
...
ORDER BY p.paid_Date";
            ...
            using (OracleCommand cmd = new OracleCommand(sql, conn))
            {
                ...
                using (OracleDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var model = new RegionalPIVStampDutyModel
                        {
