...
{
    public class AreaWiseSRPEstimationPIVRepository
    {
        private readonly string _connectionString =
            ConfigurationManager.ConnectionStrings["HQOracle"].ConnectionString;

        public List<AreaWiseSRPEstimationPIVModel> GetAreaWiseSRPEstimationPIVReport(string compId, DateTime fromDate, DateTime toDate)
        {
            var result = new List<AreaWiseSRPEstimationPIVModel>();
...
ORDER BY 1";
            ...
            using (OracleCommand cmd = new OracleCommand(sql, conn))
            {
                ...
                using (OracleDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var model = new AreaWiseSRPEstimationPIVModel
                        {
