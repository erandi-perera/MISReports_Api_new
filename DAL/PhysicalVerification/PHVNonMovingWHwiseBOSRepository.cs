...
{
    public class PHVNonMovingWHwiseBOSRepository
    {
        private readonly string _connectionString =
            ConfigurationManager.ConnectionStrings["HQOracle"].ConnectionString;

        public async Task<List<PHVNonMovingWHwiseBOSModel>> GetNonMovingWHwiseBOSAsync(
            string deptId, string warehouseCode, int repYear, int repMonth)
        {
            var result = new List<PHVNonMovingWHwiseBOSModel>();

            string sql = @"
                SELECT
                    CASE WHEN SUBSTR(T1.REASON, 0, 1) = 'N' THEN '1. Non-moving' END AS STATUS,
                    T3.PHV_DT,
                    T1.MAT_CD,
                    T2.MAT_NM,
                    T4.UOM_CD,
                    ...
                    T1.DOC_NO,
                    (T1.CNTED_QTY * T5.UNIT_COST) AS STOCK_BOOK,
                    (SELECT DEPT_NM FROM GLDEPTM WHERE DEPT_ID = :dept_id) AS REASON
                FROM INPHVDMT T1
                JOIN INPOSTMT T5 
                ...
                WHERE
                    ...
                    AND SUBSTR(T1.REASON,0,1) = 'N'
                GROUP BY 
                    CASE WHEN SUBSTR(T1.REASON,0,1) = 'N' THEN '1. Non-moving' END,
                    T1.REASON,
                    T1.MAT_CD,
                    T2.MAT_NM,
                    T4.UOM_CD,
            ...
            using (var cmd = new OracleCommand(sql, conn))
            {
                ...
                using (var reader = await cmd.ExecuteReaderAsync())
                {
                    ...
                    {
                        result.Add(new PHVNonMovingWHwiseBOSModel
                        {
