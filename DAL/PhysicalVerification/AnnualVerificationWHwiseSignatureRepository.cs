...
{
    public class AnnualVerificationWHwiseSignatureRepository
    {
        private readonly string _connectionString =
            ConfigurationManager.ConnectionStrings["HQOracle"].ConnectionString;

        public async Task<List<AnnualVerificationWHwiseSignatureModel>>
            GetAnnualVerificationWHwiseSignatureAsync(
                ...
                int repMonth)
        {
            var result = new List<AnnualVerificationWHwiseSignatureModel>();
            ...
            using (var cmd = new OracleCommand(sql, conn))
            {
                ...
                using (var reader = await cmd.ExecuteReaderAsync())
                {
                    ...
                    {
                        result.Add(new AnnualVerificationWHwiseSignatureModel
                        {
                            ...
                            PhvDate = reader["PHV_DT"] != DBNull.Value
                                                ? (DateTime?)Convert.ToDateTime(reader["PHV_DT"])
                                                : null,
